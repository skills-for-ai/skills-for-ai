...
site's own `README.md` at [`skills-on-ai.github.io/`](../../skills-on-ai.github.io/)).

## Principles and rules

- **One source, two repos, one direction.** `skills-on-ai.github.io/`
  ...
  standalone `skills-on-ai.github.io` repository is a derived artifact,
  produced by `git subtree push`, never edited directly. Changes flow one
  way: monorepo → export.
- **The repo name is not a free choice.** GitHub Pages user/organization
  sites must be served from a repository named exactly
  `{account}.github.io` — here, the `skills-on-ai` GitHub organization,
  so the standalone repo must be named `skills-on-ai.github.io`. This is
  why the nested subproject shares that exact name rather than following
  a `skills-on-ai-*` prefix — it's a GitHub platform requirement, not a
  naming inconsistency.
- **Publishing is `bin/git-subtree-push skills-on-ai.github.io`**, or the
  equivalent `make github-pages` — a thin `Makefile` target delegating to
  the POSIX shell script `bin/make-github-pages`, which runs
  `git subtree push --prefix=skills-on-ai.github.io github-pages main`
...
1. Edit `skills-on-ai.github.io/` inside the monorepo as usual; commit
   there.
2. `bin/git-subtree-push skills-on-ai.github.io` or `make github-pages`
...
configured (pointing at `skills-on-ai/skills-on-ai.github.io` on the
...
- [skills-on-ai.github.io/.git-subtree-push](../../skills-on-ai.github.io/.git-subtree-push)
