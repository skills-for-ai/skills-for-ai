...
[[sequence-diagram-skill]]'s activation bars show only in relative
...
[[plantuml-diagram-skill]] for the `@startuml` timing-diagram syntax (`concise`
...
- **Using a timing diagram when only message *order* matters, not actual
  duration** — a [[sequence-diagram-skill]] is the simpler, more
...
- [[sequence-diagram-skill]] for the related but distinct message-order-focused interaction diagram.
