---
name: black-box-testing-skill
description: Use when asked to test software from the outside, based only on specifications and inputs/outputs, without knowledge of internal code — equivalence partitioning, boundary value analysis, decision tables — as distinct from white-box testing (see white-box-testing-skill), which requires internal code knowledge.
---
...
and observable outputs matter. [[white-box-testing-skill]] is the
...
- **Boundary value analysis** — test values at the edges of input ranges
  ...
  [[boundary-testing-skill]] for this technique in depth).
...
- **Assuming black-box coverage implies code coverage** — a
  ...
  actual implementation unexercised; pair with [[white-box-testing-skill]]
  or [[mutation-testing-skill]] when code-level assurance matters.
- **Skipping boundary values in favor of only mid-range inputs** — see
  [[boundary-testing-skill]]; boundaries are disproportionately likely to
  reveal off-by-one and validation errors.

## Learn more

- [[white-box-testing-skill]] for the complementary internal-knowledge approach.
- [[boundary-testing-skill]] for boundary value analysis specifically.
- [[functional-testing-skill]] for the broader behavior-verification discipline black-box testing usually serves.
