---
name: terminal-user-interface-testing-skill
description: Use when asked to test an interactive, screen-drawing terminal application (ncurses-style, with grid/row/column layout) — simulated keystrokes, screen-scraping, escape-sequence handling — as distinct from single-invocation CLI testing (see command-line-interface-testing-skill).
---

# Terminal User Interface (TUI) Testing Skill
...
plain [[command-line-interface-testing-skill]] tool, which typically
...
- **Confusing TUI testing with plain CLI testing** — a TUI's interactive,
  ...
  that a single-invocation CLI test ([[command-line-interface-testing-skill]])
  doesn't.

## Learn more

- [[command-line-interface-testing-skill]] for the related but distinct single-invocation command testing case.
- [[vix-skill]] for a real terminal application (a text editor) this kind of testing would apply to.
