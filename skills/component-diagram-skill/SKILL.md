...
[[class-diagram-skill]] but more implementation-oriented than a
[[package-diagram-skill]].
...
- **Aggregation** — one component contains or is composed of others, but
  ...
  arrowhead (same underlying semantics as [[object-diagram-skill]]'s
...
discussions, distinct from [[deployment-diagram-skill]]'s concern with
*where* those components physically run.

## Drawing one

See [[plantuml-diagram-skill]] for the `@startuml` component-diagram syntax
...
- **Confusing a component diagram with a deployment diagram** — a
  ...
  [[deployment-diagram-skill]] shows physical runtime placement on actual
...
- **Too fine-grained a component breakdown** — showing every class as its
  ...
  [[class-diagram-skill]] in disguise, losing the architectural-overview
  value a component diagram is meant to provide.

## Learn more

- [[plantuml-diagram-skill]] for the text-based syntax to draw this diagram type.
- [[deployment-diagram-skill]] for the physical/runtime counterpart showing where components actually run.
- [[package-diagram-skill]] for an even coarser-grained organizational view.
