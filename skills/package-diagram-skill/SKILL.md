...
[[class-diagram-skill]] shows on its own.
...
See [[plantuml-diagram-skill]] for the `@startuml` package-diagram syntax
...
- **A dependency graph with cycles** — two packages depending on each
  ...
  actually fix, not just document (see [[fitness-function-testing-skill]]
...
- [[class-diagram-skill]] for the finer-grained structure inside each package.
- [[fitness-function-testing-skill]] for automating an ongoing check against unwanted package-dependency direction or cycles.
